...
// @require      https://unpkg.com/@ironkinoko/danmaku@1.3.0/dist/danmaku.umd.js
...
(function (Hls, Plyr, Danmaku) {
  ...
  function createTest(target) {
    return (test) => typeof test === "string" ? target.includes(test) || test === "*" : test.test(target);
  }
  class Runtime {
    constructor() {
      this.list = [
  ...
  function insertBTSites() {
    ...
    $(ageBlock({
      title: "\u79CD\u5B50\u8D44\u6E90\uFF1A",
      content: sites.map((site) => `<a href="${site.url}" rel="noreferrer" target="_blank" class="res_links_a">${site.title}</a>`).join("")
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="margin-right:4px;" width="1em" height="1em" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid">
  ...
  async function getVurl(href) {
    ...
    function __qpic_chkvurl_converting(_in_vurl) {
      ...
      const match_resl = vurl2.match(/^http.+\.f20\.mp4\?ptype=http\?w5=0&h5=0&state=1$/);
      return !!match_resl;
    }
...
`;
  ...
  Shortcuts.registerCommand(Commands$1.restoreSpeed, function() {
    if (this.speed !== 1) {
...
`);
  ...
  const $merge = $danmaku.find("[name='merge']");
  ...
  const start = () => {
    function run() {
      if (!player$6.media.duration)
        return requestAnimationFrame(run);
      if (!comments)
        return;
      if (player$6.localConfig.showDanmaku) {
        if (!core) {
          core = new Danmaku__default["default"]({
            container: $danmakuContainer[0],
            media: player$6.media,
            comments: adjustCommentCount(comments),
            merge: player$6.localConfig.merge
          });
        } else {
          core.reload(adjustCommentCount(comments));
          core.show();
        }
        core.speed = baseDanmkuSpeed * player$6.localConfig.danmakuSpeed;
      }
      if (player$6.localConfig.showPbp) {
        createProgressBarPower(player$6.media.duration, adjustCommentCount(comments));
      }
    }
  ...
  const adjustCommentCount = (comments2) => {
    let ret = comments2;
    ret = ret.filter((cmt) => !player$6.localConfig.danmakuFilter.some((filter) => {
      if (/^\/.*\/$/.test(filter)) {
    ...
    const mode = player$6.localConfig.danmakuMode;
    ...
    const maxLength = Math.round(3e3 / (24 * 60) * player$6.media.duration * player$6.localConfig.danmakuDensity);
  ...
  const loadEpisode = async (episodeId) => {
    ...
    state = 3 /* getComments */;
    start();
    player$6.message.info(`\u756A\u5267\uFF1A${$animes.find(":selected").text()}`, 2e3);
    player$6.message.info(`\u7AE0\u8282\uFF1A${$episodes.find(":selected").text()}`, 2e3);
    player$6.message.info(`\u5DF2\u52A0\u8F7D ${comments.length} \u6761\u5F39\u5E55`, 2e3);
  };
  ...
  const findEpisode = async (animes) => {
    ...
    if (anime) {
      ...
      if (storedEpisodeId) {
        episode = anime.episodes.find((episode2) => String(episode2.episodeId) === storedEpisodeId);
      }
      if (!episode && !isNaN(+episodeName)) {
        episode = anime.episodes.find((episode2) => episode2.episodeTitle.includes(episodeName));
      }
      if (episode) {
        state = 2 /* findEpisode */;
    ...
    player$6.message.info("\u5F39\u5E55\u672A\u80FD\u81EA\u52A8\u5339\u914D\u6570\u636E\u6E90\uFF0C\u8BF7\u624B\u52A8\u641C\u7D22");
  };
  const initEvents = (name) => {
    ...
    $danmakuSwitcher.prop("checked", player$6.localConfig.showDanmaku).on("click", () => {
      switchDanmaku();
    });
    const resizeOb = new ResizeObserver(() => {
      core == null ? void 0 : core.resize();
    });
    resizeOb.observe(player$6.$videoWrapper[0]);
    ...
    mutationOb.observe(player$6.media, { attributeFilter: ["src"] });
    player$6.initInputEvent();
    $showDanmaku.prop("checked", player$6.localConfig.showDanmaku).on("change", (e) => {
      switchDanmaku(e.target.checked);
    });
    $showPbp.prop("checked", player$6.localConfig.showPbp).on("change", (e) => {
      const chekced = e.target.checked;
      $pbp.toggle(chekced);
      player$6.configSaveToLocal("showPbp", chekced);
      if (chekced)
        autoStart();
    });
    $pbp.toggle(player$6.localConfig.showPbp || false);
    const $pbpPlayed = $pbp.find("#k-player-pbp-played-path");
    player$6.on("timeupdate", () => {
      $pbpPlayed.attr("width", (player$6.currentTime / player$6.plyr.duration || 0) * 100 + "%");
    });
    $merge.prop("checked", player$6.localConfig.merge).on("change", (e) => {
      const chekced = e.target.checked;
      $pbp.toggle(chekced);
      player$6.configSaveToLocal("merge", chekced);
      if (core)
        core.merge = chekced;
    });
    addRangeListener({
    ...
    setCheckboxGroupValue($danmakuMode, player$6.localConfig.danmakuMode);
    $danmakuMode.on("change", () => {
      const modes = getCheckboxGroupValue($danmakuMode);
      player$6.configSaveToLocal("danmakuMode", modes);
      if (core) {
        refreshDanmaku();
      }
    });
    createFilter(player$6, refreshDanmaku);
  };
  function switchDanmaku(bool) {
    bool != null ? bool : bool = !player$6.localConfig.showDanmaku;
    player$6.configSaveToLocal("showDanmaku", bool);
    $danmakuSwitcher.prop("checked", bool);
    $showDanmaku.prop("checked", bool);
    player$6.message.info(`\u5F39\u5E55${bool ? "\u5F00\u542F" : "\u5173\u95ED"}`);
    if (bool) {
      autoStart();
    } else {
      stop();
  ...
  const updateEpisodes = (anime) => {
    ...
    const html = episodes.reduce((html2, episode) => html2 + `<option value="${episode.episodeId}">${episode.episodeTitle}</option>`, "");
    $episodes.data("anime", anime);
    $episodes.html(html);
    $episodes.val("");
  };
  function autoStart() {
    if (!(player$6.localConfig.showDanmaku || player$6.localConfig.showPbp))
      return;
    switch (state) {
      case 0 /* unSearched */:
        searchAnime();
  ...
  async function setup(_player) {
    ...
    if (!info)
      return;
    videoInfo = info;
    player$6.$videoWrapper.append($danmakuContainer);
    $danmaku.insertBefore(player$6.$searchActions);
    let defaultSearchName = storageAnimeName(videoInfo.rawName) || videoInfo.name;
    initEvents(typeof defaultSearchName === "object" ? defaultSearchName.animeTitle : defaultSearchName);
    autoStart();
  }
  ...
  var css$5 = ".agefans-wrapper #relates-series .relates_series {\n  display: block;\n  padding: 4px 0px;\n}";
  n(css$5,{});
  ...
  function replacePlayer$7() {
    ...
    const fn = () => {
      ...
      if (url.origin === location.origin) {
        ...
        message.info("\u8FD9\u4E2A\u89C6\u9891\u4F3C\u4E4E\u662F\u7B2C\u4E09\u65B9\u94FE\u63A5\uFF0C\u5E76\u975E\u7531agefans\u81EA\u8EAB\u63D0\u4F9B\uFF0C\u5C06\u4F7F\u7528\u9ED8\u8BA4\u64AD\u653E\u5668\u64AD\u653E", 3e3);
        mutationOb.disconnect();
      }
    };
  ...
  function gotoPrevPart() {
    ...
    if (dom.length) {
      switchPart$a(dom.data("href"), dom);
    }
  }
  ...
  async function switchPart$a(href, $dom, push = true) {
    try {
      if (switchLoading === true)
        return;
      switchLoading = true;
      retryCount++;
      push && player$5.message.info(`\u5373\u5C06\u64AD\u653E${$dom.text()}`);
      const vurl = await getVurlWithLocal(href);
      push && player$5.message.destroy();
      const speed = player$5.plyr.speed;
      player$5.src = vurl;
      player$5.plyr.speed = speed;
      ...
      if (retryCount > 3) {
        ...
        switchPart$a(href, $dom, push);
      }
    }
  }
  function resetVideoHeight() {
    const $root = $("#ageframediv");
    const video = player$5.media;
  ...
  function addListener() {
    ...
    const update = throttle(() => {
      updateTime(player$5.currentTime);
    }, 1e3);
    player$5.on("timeupdate", () => {
      update();
    });
    player$5.on("skiperror", (_, duration) => {
      if (duration === 0) {
        updateTime(0);
      } else {
        updateTime(player$5.currentTime + duration);
      }
      window.location.reload();
    });
  ...
  function replaceHref() {
    $(".movurl:visible li a").each(function() {
      ...
      $(this).removeAttr("href").attr("data-href", href).css("cursor", "pointer").on("click", (e) => {
        e.preventDefault();
        switchPart$a(href, $(this));
      });
    });
  }
  function initPlayer(vurl) {
    player$5 = new KPlayer("#age_playfram");
    showCurrentLink(vurl);
    addListener();
    player$5.src = vurl;
  ...
  function initEvent$2() {
    player$4.on("prev", () => unsafeWindow.MacPlayer.GoPreUrl());
    player$4.on("next", () => unsafeWindow.MacPlayer.GoNextUrl());
  }
  function playModule$c() {
  ...
  runtime.register({
    domains: ["88dmw"],
    opts: [{ test: "/play", setup: www88dmwSetup, run: playModule$c }],
    search: {
      name: "\u52A8\u6F2B\u5C9B",
      search: searchAction,
  ...
  function initEvent$1() {
    player$3.on("prev", () => switchPart$9(false));
    player$3.on("next", () => switchPart$9(true));
  }
  ...
  runtime.register({
    domains: ["imomoe.live"],
    opts: [{ test: "/player", run: playModule$b }]
  });
  ...
  function replacePlayer$2(video) {
    const fn = () => {
      if (!video.src || video.src === location.href)
        return;
      player$1 = new KPlayer(".player-box", { video });
      injectEvent();
    };
  ...
  runtime.register({
    ...
    opts: [{ test: "/watch", run: playModule$8 }],
    search: {
  ...
  runtime.register({
    ...
    opts: [{ test: "/m3u8.php", runInIframe: true, run: playModule$7 }],
    search: {
      getSearchName: () => {
        return new Promise((resolve) => {
          const fn = (e) => {
  ...
  runtime.register({
    domains: [".ntyou."],
    opts: [{ test: "/play", run: playModule$6 }],
    search: {
      name: "NT\u52A8\u6F2B",
  ...
  runtime.register({
    domains: [".dm233."],
    opts: [{ test: "/play", run: playModule$5 }],
    search: {
      name: "233\u52A8\u6F2B\u7F51",
  ...
  var css$1 = ".omofun-wrapper.widescreen * {\n  visibility: hidden;\n}\n.omofun-wrapper.widescreen iframe {\n  visibility: initial;\n}";
  n(css$1,{});
