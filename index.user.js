...
`;
  ...
  Shortcuts.registerCommand(Commands$1.restoreSpeed, (() => {
    let prevSpeed = 1;
    return function() {
      if (this.speed !== 1) {
  ...
  Shortcuts.registerCommand(Commands$1.temporaryIncreaseSpeed, ...createTemporaryIncreaseSpeed());
...
`);
  ...
  const start = () => {
    function run() {
      ...
      if (player$4.localConfig.showDanmaku) {
        if (!core) {
          core = new Danmaku__default["default"]({
            ...
            merge: player$4.localConfig.merge
          });
        } else {
      ...
      if (player$4.localConfig.showPbp) {
        createProgressBarPower(player$4.media.duration, adjustCommentCount(comments));
      }
    }
  ...
  const adjustCommentCount = (comments2) => {
    let ret = comments2;
    ret = ret.filter((cmt) => !player$4.localConfig.danmakuFilter.some((filter) => {
      if (/^\/.*\/$/.test(filter)) {
    ...
    const maxLength = Math.round(3e3 / (24 * 60) * player$4.media.duration * player$4.localConfig.danmakuDensity);
  ...
  const initEvents = (name) => {
    ...
    $danmakuSwitcher.prop("checked", player$4.localConfig.showDanmaku).on("click", () => {
      switchDanmaku();
    });
    const resizeOb = new ResizeObserver(() => {
      core == null ? void 0 : core.resize();
    });
    resizeOb.observe(player$4.$videoWrapper[0]);
    ...
    player$4.on("timeupdate", () => {
      $pbpPlayed.attr("width", (player$4.currentTime / player$4.plyr.duration || 0) * 100 + "%");
    });
    $merge.prop("checked", player$4.localConfig.merge).on("change", (e) => {
      const chekced = e.target.checked;
      $pbp.toggle(chekced);
      player$4.configSaveToLocal("merge", chekced);
      if (core)
        core.merge = chekced;
    });
    addRangeListener({
  ...
  const parser = {
    "danmu.yhdmjx.com": async () => {
      ...
      player$3.src = unsafeWindow.v_decrypt(unsafeWindow.config.url, unsafeWindow._token_key, unsafeWindow.key_token);
    },
    "pro.ascepan.top": async () => {
      const video = await queryDom("video");
      video.src = "";
      player$3 = new KPlayer("#player", { eventToParentWindow: true });
      player$3.src = unsafeWindow.config.url;
    }
  };
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="margin-right:4px;" width="1em" height="1em" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid">
  ...
  var css$4 = ".agefans-wrapper #relates-series .relates_series {\n  display: block;\n  padding: 4px 0px;\n}";
  n(css$4,{});
  ...
  async function playModule$8() {
    ...
    window.addEventListener("message", (e) => {
      ...
      if (key === "getSearchName") {
        (_b = iframe.contentWindow) == null ? void 0 : _b.postMessage({ key: "getSearchName", name: $("h3.title a.title").text() }, "*");
      }
      if (key === "getEpisode") {
  ...
  runtime.register({
    ...
    search: {
      name: "NT\u52A8\u6F2B",
      search: (name) => `http://www.ntyou.cc/search/-------------.html?wd=${name}&page=1`
    }
  });
