...
`);
  ...
  const adjustCommentCount = (comments2) => {
    ...
    const maxLength = Math.round(3e3 / (24 * 60) * player$5.media.duration * player$5.localConfig.danmakuDensity);
  ...
  const loadEpisode = async (episodeId) => {
    ...
    player$5.message.info(`\u7AE0\u8282\uFF1A${$episodes.find(":selected").text()}`, 2e3);
    player$5.message.info(`\u5DF2\u52A0\u8F7D ${comments.length} \u6761\u5F39\u5E55`, 2e3);
  };
  ...
  const initEvents = (name) => {
    ...
    $danmakuSwitcher.prop("checked", player$5.localConfig.showDanmaku).on("click", () => {
      switchDanmaku();
    });
    ...
    $showDanmaku.prop("checked", player$5.localConfig.showDanmaku).on("change", (e) => {
  ...
  function switchDanmaku(bool) {
    bool != null ? bool : bool = !player$5.localConfig.showDanmaku;
  ...
  var css = ".omofun-wrapper.widescreen * {\n  visibility: hidden;\n}\n.omofun-wrapper.widescreen iframe {\n  visibility: initial;\n}";
  n(css,{});
