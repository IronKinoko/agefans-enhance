...
(function ($$1, Plyr, Hls) {
  ...
  class Runtime {
      ...
      run() {
          let setupList = [];
          let runList = [];
          const list = this.list.filter(({ domains }) => domains.some(createTest(location.origin)));
          list.forEach(({ opts }) => {
...
`).insertAfter($__default['default']('.baseblock:contains(网盘资源)'));
  ...
  function genUrlItem(title, content = '加载中...') {
    const download = [$__default['default']('#detailname a').text(), title].join(' ');
    const contentHTML = content.startsWith('http') ? `<a href="${content}" download="${download}">${content}</a>` : content;
    return `<div>
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="margin-right:4px;" width="1em" height="1em" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid">
    ...
    list.forEach(item => {
      ...
      async function _getUrl() {
        try {
          ...
          const download = [$__default['default']('#detailname a').text(), item.title].join(' ');
  ...
  function showSetting() {
    ...
    const $usePreview = $__default['default'](`<label class="agefans-setting-item"><input type="checkbox" />启用番剧信息预览（鼠标悬浮时预览番剧基础信息）</label>`);
    $usePreview.find('input').prop('checked', setting.usePreview).on('change', e => {
      setSetting('usePreview', e.target.checked);
    });
    const $stopUseKPlayer = $__default['default'](`<label class="agefans-setting-item"><input type="checkbox" /><b>暂时</b>使用原生播放器</label>`);
    ...
    modal({
      ...
      content: $__default['default']('<div></div>').append(alert('这些配置需要刷新页面才能生效'), $usePreview, $stopUseKPlayer)
    });
  }

  function settingModule() {
    ensureDefaultSetting();
    $__default['default']('<a>设置</a>').on('click', showSetting).insertBefore('.loginout a');
  }

  function pagePreview(trigger, previewURL) {
    if (!getSetting('usePreview')) return;
    const $popover = $__default['default'](`<div class='page-preview' style="display:none">
       <div class="page-preview-center">${loadingIcon}加载中...</div>
     </div>`).appendTo($__default['default'](trigger));
    /**
     * @param {MouseEvent} e
     */
    ...
    let isLoaded = false;
    let timeId = null;
    $__default['default'](trigger).addClass('page-preview-trigger').on('mouseenter', e => {
      $popover.show();
      caclPosition(e);
      if (isLoaded) return;
      clearTimeout(timeId);
      timeId = setTimeout(async () => {
        ...
        let {
          img,
          info
        } = session.getItem(previewURL, {});

        if (!info) {
          const $root = $__default['default'](await fetch(previewURL).then(r => r.text()));
          img = $root.find('#container > div.div_left > div:nth-child(1) > div > img').css({
            display: 'block',
            width: 256,
            height: 356
          }).prop('outerHTML');
          info = $root.find('#container > div.div_left > div:nth-child(2) > div > div').width(256);
          info.find('.blocktitle.detail_title1').text($root.find('.detail_imform_name').text());
          info = info.prop('outerHTML');
          session.setItem(previewURL, {
            img,
            info
          });
        }
  ...
  function renderHistroyStyle() {
    $__default['default']('<style/>').html(`.movurl li a:visited { color: red; }`).appendTo('head');
  }

  function detailModule() {
    renderHistroyStyle();
    $__default['default']('.div_left li > a:nth-child(1), .ul_li_a4 li > a:nth-child(1)').each((_, anchor) => pagePreview(anchor.parentNode, anchor.href));
  }

  var css$6 = ".agefans-wrapper #history {\n  background: #202020;\n  border: 4px solid #303030;\n}\n.agefans-wrapper #history .history-list {\n  padding: 8px;\n  display: flex;\n  flex-wrap: wrap;\n}\n.agefans-wrapper #history .history-item {\n  width: 115px;\n  display: inline-block;\n  margin: 4px;\n}\n.agefans-wrapper #history .history-item img {\n  width: 100%;\n  border-radius: 2px;\n}\n.agefans-wrapper #history .history-item .desc .title {\n  overflow: hidden;\n  white-space: nowrap;\n  text-overflow: ellipsis;\n  font-size: 14px;\n  margin: 4px 0;\n}\n.agefans-wrapper #history .history-item .desc .position {\n  font-size: 14px;\n}\n@media (max-width: 480px) {\n  .agefans-wrapper #history .history-list {\n    display: grid;\n    grid-template-columns: repeat(3, 1fr);\n    grid-gap: 8px;\n  }\n  .agefans-wrapper #history .history-item {\n    width: auto;\n    margin: 0;\n    display: block;\n    min-width: 0;\n  }\n}";
  n(css$6,{});
  ...
  function renderHistoryList() {
    $__default['default']('#history').html('').append(() => {
      ...
      let html = '';
      histories.forEach(o => {
        html += `<a class="history-item" href="${o.href}" data-detail-href="/detail/${o.id}">
          <img
  ...
  function renderHistoryPage() {
    ...
    const $hisNavBtn = $__default['default'](`<a class="nav_button">历史</a>`).insertBefore('#nav form').on('click', e => {
  ...
  function popover(target, content) {
    const $target = $__default['default'](target);
    const $content = $__default['default'](`<div class="k-popover-overlay"><div class="k-popover-content">${content}</div></div>`);
    let timeID = null;
...
`;
  ...
  class Message {
    ...
    info(message, duration = 1500) {
      ...
      return new Promise(resolve => {
        $__default['default'](`<div class="k-player-message-item"></div>`).append(message).hide().appendTo(this.$message).fadeIn(150).delay(duration).fadeOut(150, function () {
  ...
  class KPlayer {
    ...
    constructor(selector, opts = {}) {
      ...
      const $pip = $__default['default'](pipHTML);
      const $video = $__default['default']((_opts$video = opts.video) !== null && _opts$video !== void 0 ? _opts$video : '<video />').attr('id', 'k-player');
      ...
      this.plyr = new Plyr__default['default']('#k-player', {
        ...
        controls: ['play', 'progress', 'current-time', 'duration', 'mute', 'volume', 'pip', 'fullscreen'],
        storage: false,
        seekTime: 5,
      ...
      this.$videoWrapper.append($loading).append($error).append($pip).append($progress).append($header);
    ...
    _initEvent() {
      ...
      let timeId = null;
      $__default['default'](".plyr--video input[type='range']").on('mousedown', function () {
        clearInterval(timeId);
        let i = 0;
        timeId = setInterval(() => {
          $__default['default'](this).removeClass().addClass(`shake-${i++ % 2}`);
        }, 100);
      });
      $__default['default'](".plyr--video input[type='range']").on('mouseup', function () {
        clearInterval(timeId);
    ...
    _injectSnapshot() {
      ...
      $__default['default']($__default['default']('#plyr__snapshot').html()).insertBefore('[data-plyr="fullscreen"]').on('click', () => {
        this._snapshot();
      });
    }
    /** @private */
    ...
    eventToParentWindow() {
      const evnetKeys = ['prev', 'next', 'enterwidescreen', 'exitwidescreen', 'skiperror', 'progress', 'playing', 'play', 'pause', 'timeupdate', 'volumechange', 'seeking', 'seeked', 'ratechange', 'ended', 'enterfullscreen', 'exitfullscreen', 'captionsenabled', 'captionsdisabled', 'languagechange', 'controlshidden', 'controlsshown', 'ready', 'loadstart', 'loadeddata', 'loadedmetadata', 'canplay', 'canplaythrough', 'stalled', 'waiting', 'emptied', 'cuechange', 'error'];
  ...
  function showCurrentLink(vurl) {
    ...
    const title = [$__default['default']('#detailname a').text(), getActivedom$2().text()].join(' ');
...
`).insertBefore($__default['default']('.baseblock:contains(网盘资源)'));
  ...
  function videoJumpHistoryPosition() {
    ...
    if (((_his$get = his.get(id)) === null || _his$get === void 0 ? void 0 : _his$get.time) > 3) {
      player$4.currentTime = his.get(id).time;
      player$4.message.info(`已自动跳转至历史播放位置 ${parseTime(his.get(id).time)}`);
    }
  }
  ...
  function addListener() {
    ...
    player$4.plyr.once('canplay', () => {
      if (player$4.localConfig.continuePlay) {
    ...
    player$4.on('timeupdate', () => {
      if (Math.floor(player$4.currentTime) % 3 === 0) {
  ...
  function replaceHref() {
    $__default['default']('.movurl:visible li a').each(function () {
      ...
      $__default['default'](this).removeAttr('href').attr('data-href', href).css('cursor', 'pointer').on('click', e => {
        e.preventDefault();
        switchPart$7(href, $__default['default'](this));
      });
    });
  }
  /** @type {KPlayer} */
  ...
  async function showRelatesSeries() {
    ...
    const $series = $__default['default'](info).find('li.relates_series');
...
`).insertAfter($__default['default']('.baseblock:contains(简介：)')).find('ul').append($series);
  }
  ...
  function playModule$9() {
    ...
    initGetAllVideoURL();
    showRelatesSeries();
    $__default['default']('.ul_li_a8 > .anime_icon1 > a:nth-child(1)').each((_, anchor) => pagePreview(anchor.parentElement, anchor.href));
  }

  function rankModule() {
    $__default['default']('.div_right_r_3 ul > li > a').each((_, anchor) => pagePreview(anchor, anchor.href));
  }

  function recommendModule() {
    $__default['default']('ul.ul_li_a6 > li > a').each((_, anchor) => pagePreview(anchor.parentNode, anchor.href));
  }
  ...
  runtime.register(['age.tv', 'agemys', 'agefans'], [{
    test: '*',
    run: () => {
  ...
  function initEvent$2() {
    player$3.on('prev', () => window.MacPlayer.GoPreUrl());
    player$3.on('next', () => window.MacPlayer.GoNextUrl());
  }
  ...
  function replacePlayer$3() {
    new KPlayer('#dplayer', {
      video: $__default['default']('video')[0],
  ...
  function playModule$7() {
    ...
    window.addEventListener('message', e => {
      ...
      if (key === 'prev') switchPart$6(false);
      if (key === 'next') switchPart$6(true);

      if (key === 'enterwidescreen') {
        $__default['default']('body').css('overflow', 'hidden');
        $__default['default']('#playbox iframe').css({
          position: 'fixed',
          left: 0,
          right: 0,
          bottom: 0,
      ...
      if (key === 'exitwidescreen') {
        ...
        $__default['default']('#playbox iframe').removeAttr('style');
      }
    });
    window.addEventListener('keydown', e => {
      if (document.activeElement !== document.body) return;
      $__default['default']('#playbox iframe')[0].focus();
  ...
  runtime.register(['yhdm.so', 'yinghuacd.com'], [{
    test: ['/v'],
    run: playModule$7
  }, {
    test: ['vid'],
  ...
  function replacePlayer$2() {
    ...
    const fn = () => {
      ...
      if (videoURL) {
        player$2 = new KPlayer('#yh_playfram');
  ...
  function initEvent$1() {
    player$2.on('prev', () => switchPart$5(false));
    player$2.on('next', () => switchPart$5(true));
  }
  ...
  function switchPart$3(next) {
      player.on('prev', () => {
          $__default['default']('.meida-content-main-window-right-series-list-volume-active')[next ? 'next' : 'prev']()
              .prev()
              .find('a')[0]
              ?.click();
      });
  ...
  runtime.register(['new-ani.me'], [{ test: '/watch', run: playModule$4 }]);
  ...
  runtime.register(['danmu.4dm.cc'], [{ test: '/m3u8.php', runInIframe: true, run: playModule$3 }]);
  ...
  runtime.register(['.ntyou.'], [{ test: '/play', run: playModule$2 }]);
  ...
  runtime.register(['.dm233.'], [{ test: '/play', run: playModule$1 }]);
